...
class PersonManager:
    ...
    def __init__(self):
        ...
        inFp = open('./sample.txt', 'r', encoding='utf-8')

        inList = inFp.readlines()

        self.datacnt = len(inList)

        for data in inList:
            tmp = data.split()
            if tmp[0] == '1':
                ptmp = Person(tmp[1], int(tmp[2]))
                self.memberdict[ptmp.No] = ptmp
            elif tmp[0] == '2':
                ptmp = Employee(tmp[1], int(tmp[2]), tmp[3], tmp[4], tmp[5])
                self.memberdict[ptmp.No] = ptmp
        inFp.close()
    ...
    def save(self):
        ...
        outFp = open('./sample.txt', 'w', encoding='utf-8')

        # for k, p in self.memberdict.items():
        for p in self.memberdict.values():
            if type(p) == Person:
                outStr = '{} {} {}\n'.format(1, p.getName(), p.getAge())
                outFp.writelines(outStr)
            elif type(p) == Employee:
                outStr = '{} {} {} {} {} {}\n'.format(2, p.getName(), p.getAge(), p.getCompany(), p.getCID(),
                                                      p.getPhone())
                outFp.writelines(outStr)
